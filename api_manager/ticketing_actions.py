...
from fastapi import FastAPI,APIRouter, Form, UploadFile, File,HTTPException,Depends
...
async def ticketing_update_ticket(data: ticketing_model.TicketingUpdateTicketParams,current_user = Depends(get_current_user)):
    try:   
        ...
        if can_update_ticket(current_user,result) == False:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,detail="You donot have access to update")
